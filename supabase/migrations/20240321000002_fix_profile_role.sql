...
INSERT INTO roles (name, description, role_level, is_firm_specific, permissions) VALUES
    ('attorney', 'Licensed attorney with full access to legal services', 'professional', true,
    '{"can_manage_cases": true, "can_manage_documents": true}'::jsonb),
    ('paralegal', 'Legal professional supporting attorneys', 'support', true,
    '{"can_view_cases": true, "can_manage_documents": true}'::jsonb),
    ('client', 'Client with access to their own cases and documents', 'support', true,
    '{"can_view_cases": true}'::jsonb),
    ('admin', 'System administrator with full access', 'system', false,
    '{"can_manage_platform": true, "can_manage_firms": true}'::jsonb)
ON CONFLICT (name) DO NOTHING;

-- Update the stored procedure to handle role_id
