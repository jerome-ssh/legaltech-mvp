...
DROP FUNCTION IF EXISTS update_profile_with_related(text, text, text, text, text, text, text, integer, text, text, text, text, text, boolean, jsonb);

-- Create or replace the profile update procedure with improved JSONB handling
...
begin
  ...
  if p_professional_ids is not null and jsonb_array_length(p_professional_ids) > 0 then
    ...
    loop
      ...
      values (
        v_profile_id,
        coalesce(v_professional_id->>'country', ''),
        coalesce(v_professional_id->>'state', ''),
        coalesce(v_professional_id->>'professional_id', ''),
        coalesce((v_professional_id->>'year_issued')::integer, 0),
        coalesce(v_professional_id->>'document_url', '')
      );
    end loop;
  end if;
