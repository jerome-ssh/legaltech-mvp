...
CREATE OR REPLACE FUNCTION create_supabase_user(p_clerk_user_id text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Update the profile update procedure to handle user creation
...
begin
  -- Create or get Supabase user ID
  v_supabase_user_id := create_supabase_user(p_clerk_user_id);

  -- Get or create user mapping
  insert into user_mappings (clerk_user_id, supabase_user_id)
  values (p_clerk_user_id, v_supabase_user_id)
