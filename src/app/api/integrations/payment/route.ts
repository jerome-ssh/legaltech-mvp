...
export async function GET() {
  try {
    ...
    // Get payment integrations for the firm
    ...
    console.error('Error in GET /api/integrations/payment:', error);
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      console.error('Error adding payment integration:', error);
      return NextResponse.json({ error: 'Failed to add integration' }, { status: 500 });
    }

    return NextResponse.json(integration);
  } catch (error) {
    console.error('Error in POST /api/integrations/payment:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
} 