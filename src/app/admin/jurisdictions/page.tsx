...
export default function JurisdictionAdminPage() {
  ...
  const fetchJurisdictions = async () => {
    ...
    setError(null);
    try {
      const { data, error } = await supabase.from("jurisdictions").select("id, name, description");
  ...
  const handleEdit = (jur: any) => {
    setEditingId(jur.id);
    setEditForm({ name: jur.name, description: jur.description });
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const { error } = await supabase.from("jurisdictions").update({
        name: editForm.name,
  ...
  return (
    <div className="container mx-auto py-8">
      <h1 className="text-2xl font-bold mb-6">Jurisdictions (Admin)</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card key={j.id} className="relative">
            {editingId === j.id ? (
              ...
              <>
                ...
                <CardContent>
                  <div className="text-sm text-gray-500 mb-2">{j.description}</div>
                  <div className="flex gap-2 mt-2">
                    <Button size="icon" variant="outline" onClick={() => handleEdit(j)}><Pencil className="w-4 h-4" /></Button>
                    <Button size="icon" variant="destructive" onClick={() => setShowDelete(j.id)}><Trash className="w-4 h-4" /></Button>
                  </div>
                </CardContent>
                {showDelete === j.id && (
                  <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center z-10">
                    <div className="bg-white rounded-lg p-4 shadow-lg flex flex-col gap-4">
                      <div>Delete <span className="font-bold">{j.name}</span>? This cannot be undone.</div>
                      <div className="flex gap-2 justify-end">
                        <Button size="sm" variant="outline" onClick={() => setShowDelete(null)} disabled={deleteLoading === j.id}><X className="w-4 h-4" /> Cancel</Button>
                        <Button size="sm" variant="destructive" onClick={() => handleDelete(j.id)} disabled={deleteLoading === j.id}>{deleteLoading === j.id ? <Loader2 className="w-4 h-4 animate-spin" /> : "Delete"}</Button>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {jurisdictions.length === 0 && <div className="col-span-full text-gray-500">No jurisdictions found.</div>}
      </div>
    </div>
  );
} 